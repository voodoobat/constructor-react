...
function CreateForm ({ dispatch }) {
  ...
  const [isInvalid, setInvalid] = useState(or(true, [invalidCols, invalidRows, invalidName, invalidEven]))
  ...
  const check = () => {
    setInvalidName(name.replace(/\s/g, '').length < MIN_SCHEME_NAME_LEN || name.length > MAX_SCHEME_NAME_LEN)
    ...
    <form
      ...
      <Field
        ...
        <Input
          ...
          setter={setName}
          value={name} />
          <div className={classNames(scss.error_msg, invalidNameMin ? scss.visible : '')}>
            напишите хоть что-нибудь
          </div>
          <div className={classNames(scss.error_msg, invalidNameMax ? scss.visible : '')}>
            ну это уже слишком
          </div>
      </Field>
      <Field
        ...
        <Number
          ...
          setter={setRows}
          value={rows} />
        <div className={classNames(scss.error_msg, invalidRows ? scss.visible : '')}>от {min} до {max}</div>
        <div className={classNames(scss.error_msg, invalidEven ? scss.visible : '')}>
      ...
      <Field
        ...
        <Number
          ...
          setter={setCols}
          value={cols} />
        <div className={classNames(scss.error_msg, invalidCols ? scss.visible : '')}>
          от {min} до {max}
        </div>
      </Field>
      <Field>
