...
function LegendBox({ className, schemeCanvas, schemeLegends, isPreview }) {
  const [legends, setLegends] = useState(
    fn.getSchemeLegends(schemeLegends, schemeCanvas, isPreview)
  )
    ...
    <>
      {!!legends.length && (
        <div className={classNames(className, scss._)}>
          <h3 className={scss.caption}>Условные обозначения</h3>
          {legends.map((legend) => (
            <div key={legend.id} className={scss.legend}>
              <Legend legend={legend} isPreview={isPreview} />
            </div>
          ))}
        </div>
      )}
